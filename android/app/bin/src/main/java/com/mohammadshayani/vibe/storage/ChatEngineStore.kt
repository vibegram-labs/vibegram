...
internal object ChatEngineStore {
  ...
  fun setConfig(context: Context, payload: Map<String, Any?>) {
    ...
    prefs(context).edit().putString(KEY_CONFIG, JSONObject(sanitized).toString()).commit()
  }
  ...
  fun updateConfig(context: Context, changes: Map<String, Any?>) {
    val current = getConfig(context).toMutableMap()
    for ((key, value) in changes) {
      if (value != null) {
        ...
        if (SecureKeyStore.sensitiveKeys.contains(key)) {
          SecureKeyStore.deleteSecret(context, key)
        }
      }
    }
    setConfig(context, current)
  }
  ...
  fun clearConfig(context: Context) {
    prefs(context).edit().remove(KEY_CONFIG).commit()
    SecureKeyStore.sensitiveKeys.forEach { SecureKeyStore.deleteSecret(context, it) }
  }
