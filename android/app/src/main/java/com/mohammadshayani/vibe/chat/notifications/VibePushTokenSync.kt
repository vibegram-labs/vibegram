...
internal object VibePushTokenSync {
  ...
  fun refreshFirebaseTokenIfAvailable(context: Context, reason: String) {
    try {
      ...
      if (!taskClass.isInstance(task)) return
      val listener = Proxy.newProxyInstance(
        listenerClass.classLoader,
        arrayOf(listenerClass),
      ) { _, method, args ->
        if (method.name == "onComplete" && args?.isNotEmpty() == true) {
          handleFirebaseTokenTask(context.applicationContext, args[0], reason)
        }
        null
      }
      task.javaClass.methods.firstOrNull {
        it.name == "addOnCompleteListener" &&
          it.parameterTypes.size == 1 &&
          it.parameterTypes[0].isAssignableFrom(listenerClass)
      }?.invoke(task, listener)
    } catch (_: ClassNotFoundException) {
      Log.d("VibePushTokenSync", "Firebase Messaging unavailable reason=$reason")
    } catch (t: Throwable) {
      Log.w("VibePushTokenSync", "Firebase token refresh failed reason=$reason ${t.message}", t)
    }
  }
  ...
  private fun handleFirebaseTokenTask(context: Context, task: Any, reason: String) {
    try {
      val isSuccessful = task.javaClass.getMethod("isSuccessful").invoke(task) as? Boolean ?: false
      if (!isSuccessful) {
        Log.d("VibePushTokenSync", "Firebase token task not successful reason=$reason")
        return
      }
      val token = task.javaClass.getMethod("getResult").invoke(task)?.toString()?.trim().orEmpty()
      if (token.isBlank()) return
      VibeNativeCallStore.setFcmToken(context, token)
      Log.d("VibePushTokenSync", "Firebase token refreshed reason=$reason len=${token.length}")
      syncStoredPushTokens(context, reason = "firebase-token-$reason")
    } catch (t: Throwable) {
      Log.w("VibePushTokenSync", "Firebase token task handling failed reason=$reason ${t.message}", t)
    }
  }
